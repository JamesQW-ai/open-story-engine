...
folder=Path(tempfile.mkdtemp(prefix='ose-prose-'))
Path('/private/tmp/ose-prose-current').write_text(str(folder))
...
for kind,action in actions.items():
    request={'parent_branch_id':parent,'text':action,'request_id':'prose-v1-'+kind}
