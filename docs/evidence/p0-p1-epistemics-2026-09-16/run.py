...
folder=Path(tempfile.mkdtemp(prefix='ose-epistemics-'))
Path('/private/tmp/ose-epistemics-current').write_text(str(folder))
...
batch=sys.argv[1] if len(sys.argv)>1 else 'v1'
report={'batch':batch,'promptVersion':catalog_version(),'mainDatabaseBefore':before,'perTurnMaxTransportCalls':18,'turns':[]}
for kind,action in actions.items():
    request={'parent_branch_id':parent,'text':action,'request_id':'epistemics-'+batch+'-'+kind}
    started=time.monotonic(); chunks=[]; first=[]
    def delta(text):
        if not first: first.append(round((time.monotonic()-started)*1000))
        chunks.append(text)
        if len(chunks) == 1: print(kind + ': first text', flush=True)
    try:
