...
source = Path('/private/tmp/ose-epistemics-current').read_text().strip()
destination = root / (sys.argv[1] if len(sys.argv)>1 else 'batch-1')
...
(destination / 'readable-scenes.md').write_text('\n'.join(readable))
