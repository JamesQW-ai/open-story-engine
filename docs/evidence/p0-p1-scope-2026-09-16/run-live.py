...
folder=Path(tempfile.mkdtemp(prefix='ose-p0p1-'))
Path('/private/tmp/ose-scope-final').write_text(str(folder))
...
for kind,action in actions.items():
    request={'parent_branch_id':parent,'text':action,'request_id':'scope-v2-'+kind}
