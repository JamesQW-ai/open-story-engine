...
`npm run play` 会创建一个新会话，默认写入 `data/open-story-engine.sqlite`。CLI 接受许川的自然语言行动，例如“许川俯身检查十七号柜的铜牌”或“他请求姜序带路”。mock `ActionParser` 只会映射到故事包中既有的稳定 ID；它不确定时返回澄清，不会写入事件。
