...
`content/annotations/rainy-waiting-room.v0.1.json` 保留从母本到结构化剧情锚点的溯源记录；`content/packages/rainy-waiting-room/0.1.0.json` 是引擎实际加载的候选 `StoryPackage`。稳定 ID、状态旗标、检定与结局条件均以候选故事包为准，不由叙事模型再次推断。
