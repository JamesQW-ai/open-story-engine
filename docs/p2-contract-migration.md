...
- `web/tests/desktopReaderIntegration.test.mjs` 启动真实构建 reader-server，导出官方十万字长篇，覆盖七身份/七入口、预设方向、自由文本、失败重试、永久后果、自动终局与显式收束回执、结局后续写阻断、刷新回看和分支恢复。

## 验收边界

契约与桌面集成测试只证明当前 HTTP/UI 边界和冻结运行包字段可重复验证，不宣称真实模型生成质量、长篇叙事质量或完整产品验收。桌面夹具中的自动终局、审查和收束结果是明确标记的结构化交互注入，用于区分前端状态处理与契约失败；它不替代 P0 终局规则，真实模型未运行。
