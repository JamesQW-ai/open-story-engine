    ...
    def catalog(self, package: dict[str, Any], path: Path | None = None) -> dict[str, Any]:
        ...
        return {
            ...
            **({
            ...
            # The package retains NPC cards for narrative continuity, while
            # the player catalog exposes only explicitly reviewed identities.
            "characters": [character for character in package["characters"] if character.get("id") in playable_ids],
            "supporting_characters": [
                {key: character[key] for key in ("id", "name", "roleGroup", "identitySummary") if key in character}
                for character in package["characters"]
                if character.get("rosterVisible") and character.get("id") not in playable_ids
            ],
            "locations": package["locations"], "items": package["items"],
        }
