...
def known_relationships(nodes, people, player_name):
    ...
    # Reviewed notes can refer back to an established relationship once both
    # people have appeared. They remain branch-local, quoted and non-authoritative.
    for node in nodes:
        known = {p['name']: p['id'] for p in people if p['first_page'] <= node['sequence'] + 1}
        for item in node.get('readerOutcome', {}).get('relationships', []):
            if (item.get('source') in known and item.get('target') in known
