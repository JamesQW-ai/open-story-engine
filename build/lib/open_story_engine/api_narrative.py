...
from .api_reader_quality import (validate_outcome, continuity_check, reading_history,
                                 action_requirements, validate_action_requirements)
...
class PlayerNarrativePlanner(LlmPlanner):
    ...
    def _check_action_authority(self, context, action, contract, raw, observations):
        completion = complete_with_retry(self.gateway, 'complete_json', [
            ...
            {'role': 'user', 'content': json.dumps({
                ...
                'priorState': context['parent']['branchState'],
                'steps': contract['steps'], 'stateChanges': contract['stateChanges'],
            }, ensure_ascii=False)},
        ])
        raw.append(completion.raw_response)
        observations.extend({**o, 'generationStage': 'action_authority'} for o in completion.observations)
        return reader_actions.validate_authority(parse_json_content(completion.content), contract, action, context['parent']['narrativeText'])
    ...
    def _prompt(self, context, selected, state, repair, terminal_source_branch=False):
        ...
        if context['contract'].get('openingContext'):
            ...
            event_brief += ('\n入场时的角色知识与关系（是前史，不得用于重置本局后来已确认的变化）：'
                            ...
                            + '\n当前物品与人物状态以本回合状态为准：' + json.dumps(state, ensure_ascii=False)
        ...
        if context.get('resultContract'):
            ...
            event_brief += '\n已发生重大结果的原始正文（复述必须与实际经过一致）：' + json.dumps(consequences.planning_context(context)['criticalHistory'], ensure_ascii=False)
        if current:
            ...
            if consequences.enabled(context['package']):
                ...
                plan_messages = [
                    ...
                    {'role': 'user', 'content': json.dumps({**consequences.planning_context(context),
                        'input': player_action(context, selected), 'requirements': requirements}, ensure_ascii=False)},
                ]
                for contract_attempt in range(2):
                    planning = complete_with_retry(self.gateway, 'complete_json', plan_messages)
            ...
            for attempt in range(3):
                ...
                try:
                    ...
                    if api_routes.role_name(context['package'], context['parent']['branchState']):
                        continuity_check(body, name, '\n'.join(n.get('narrativeText', '') for n in context['lineage']))
                    ...
                    if current:
                        ...
                        review = complete_with_retry(self.gateway, 'complete_json', [
                            ...
                            {'role': 'user', 'content': json.dumps({'facts': '正文中的你指' + name + '。' + api_routes.fact_sheet(context['package'], context['parent']['branchState'], interlude)
                                ...
                                'player_action': player_action(context, selected), 'previous': '\n\n'.join(n.get('narrativeText', '') for n in context['lineage'])[-10000:],
                    ...
                    if not current and context['contract'].get('continuityContract'):
                        ...
                        if result_contract:
                            observation_messages = [
                                {'role': 'system', 'content': reader_actions.OBSERVE_RULES},
                                {'role': 'user', 'content': json.dumps({
                                    'registry': reader_actions.registry(context['package'], context['parent']['branchState'], result_contract['introductions']),
                                    'state': context['parent']['branchState'],
                                    'previous': context['parent']['narrativeText'],
                            ...
                            for extraction_attempt in range(2):
                                observation = complete_with_retry(self.gateway, 'complete_json', observation_messages)
                                ...
                                try:
                                    events = reader_actions.validate_observations(parse_json_content(observation.content), body)
                                    break
                                except ValueError as error:
                        ...
                        review_messages = [
                            {'role': 'system', 'content': render_prompt('reader.action_review') + (consequences.REVIEW_RULES if result_contract else '')},
                            {'role': 'user', 'content': json.dumps({'player': name, 'requirements': requirements,
                                ...
                                'continuity': consequences.planning_context(context),
                                'authoritativeState': context['parent']['branchState'],
                                'goals': consequences.goals_for(context['package'], context['contract'], context['parent']['branchState']),
                                'previous': context['parent']['narrativeText'][-6000:], 'draft': {f'P{i+1}': p for i, p in enumerate(body.split('\n\n'))}}, ensure_ascii=False)},
                        ]
                        action_review = complete_with_retry(self.gateway, 'complete_json', review_messages)
                        ...
                        if result_contract:
                            ...
                            except consequences.ConsequenceEvidenceError as error:
                    ...
                    if 0 < count <= MAX_CHAPTER_CHARACTERS:
                        ...
                        if result_contract:
                            material += '\n结果契约与永久状态：' + json.dumps({'contract': result_contract, 'state': resolved_state}, ensure_ascii=False)
