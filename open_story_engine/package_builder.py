...
ITEM_SUFFIXES = ("录音笔", "图纸", "铜牌", "通行证", "门卡", "钥匙", "日志", "扳手", "文件袋", "对讲机", "时刻表", "手机", "档案", "账本", "信件", "令牌", "玉佩", "长剑", "药瓶", "灯")
...
def _recognized_source_names(text: str) -> List[str]:
    ...
    """
    pattern = re.compile("([" + SURNAME_CHARACTERS + r"][\u4e00-\u9fff])")
    ...
    labels = {match.group(1) for match in re.finditer(r"(?:名叫|叫做|写着[：:]|姓名[：:])([" + SURNAME_CHARACTERS + r"][\u4e00-\u9fff])", text)}
    return sorted((
...
def _looks_like_nonperson(value: str) -> bool:
    return _looks_like_location(value) or value in {"司机", "主管", "医生", "老师", "警察", "师傅", "老板", "父亲", "母亲", "程序", "安全"} or value.endswith(("灯", "门", "车", "厅", "站", "室", "线", "图", "表", "声", "气", "水", "路", "钟", "月", "天", "年"))
...
def _plot_sentence(text: str, names: Sequence[str]) -> str | None:
    ...
    for sentence in sentences:
        ...
        if marker and not re.search(r"想[^。！？]{0,8}" + re.escape(marker), sentence) and any(name in sentence for name in names):
            return sentence
    return None
...
def audit_story_package(path: Path, analysis: Dict[str, Any], package: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if len(entry_character_ids) != len(major_names) or any(
