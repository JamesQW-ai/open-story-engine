...
SCENES = {'唐栖': [('维修隧道',
 ...
 '陈砚': [('站务室',
        ...
        ('候车厅',
         '候车厅',
         '在众人面前',
         '你和其他三人沿维修隧道、站务室返回候车厅。许川把录音笔交给唐栖，她把采购单、验收单和门禁记录铺在长椅上。按玩家选择听取逐项质问或先核对记录，你面对姜序否认签名和旧录音。允许你解释、沉默或提出核对，但不能写成证据已被销毁、众人原谅或你已被定罪。列车仍停着，司机来到众人面前。'),
        ('候车厅',
         '候车厅',
         '留下来面对',
         '司机当面宣布列车继续停站并按事故程序上报，姜序写下真实故障和救援记录，唐栖保留原始证据。若玩家选择说明经手记录，写你按已知事实回答；若选择交出岗位，则写你让司机和维修人员按程序处理，自己留在现场接受核查，不强迫忏悔。回扣零点前放行的压力：这次人员安全先于时刻表。现场危机结束，最终责任仍待调查，不写拘捕、定罪或新危机。')]}
...
def directions(package, state):
    ...
    return [{'id': f'direction_player_{step+1}_{i+1}', 'title': title,
             'summary': CHOICE_HINTS[name][step],
...
def prepare_direction(package, parent, selected, player_direction=None):
    ...
    return {**{k: v for k, v in selected.items() if k not in ('canonicalBeatId', 'followupBeatId', 'rejoinTargetId', 'arcId', 'directionLevel')},
            'statePatch': scene_patch(package, parent['branchState'], action)}


def check_scene_result(package, before, text):
    ...
    for prop in ('撬棍', '复印机', '备用钥匙', '墨水颜色', '新的刮痕'):
        if prop in text:
            raise ValueError('本场景没有提供这些工具或可取证细节：' + prop + '。只使用已提供的材料和工具')
    if step == 2 and re.search(r'回勾|编号章|编号跳|采购申请页|(?:数量|数字)[^。！？]{0,20}(?:不一致|对不上|差额)|(?:另一|两)组数字|(?:签名|自己的字)[^。！？]{0,25}(?:倾斜|工整|笔画|末笔|这么斜)', text):
...
def fact_sheet(package, before):
    ...
    facts = ('固定人物：许川、陈砚、姜序为男性，NPC用“他”；唐栖为女性，NPC用“她”；当前玩家始终用“你”。'
             ...
             '不新增具名人物、证人、过去的会面、来电或纸面的日期数字。原文只提供的异常才是证据；不要用推理补造证据。'
             '没有撬棍、备用钥匙或复印机。不编造刮痕、门框凹痕、墨水颜色、笔画形状或设备型号来证明责任。')
    step = completed_steps(before)
    if step >= 2 or name != '唐栖':
    ...
    if step == STEPS-1:
        ...
        facts += '证据保管方式固定：文件袋和录音笔由唐栖贴身保管，日志由姜序保管，司机负责上报现场情况；不要新增钥匙、上锁的箱柜或把原件交给别的人。司机有原著明确的随车应急卫星终端，普通手机无信号不妨碍他按事故程序上报；不编造外部回电。结局的真实日志可写：二十三点四十七分，排水泵故障未完成维修；二十三点五十五分，发现人员受困；零点零三分，救援完成。调查会继续，不能宣称已定罪或隐患已修复。'
    return facts


def check_reviewed_ending(package, before, body, ending):
    ...
    if ending.get('final_location') != current[0]:
        raise ValueError('章末玩家实际地点与已确认事件不符，本章必须停在' + current[0])
    step = completed_steps(before)
