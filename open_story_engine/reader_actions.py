...
class ActionEvidenceError(ValueError):
    """A malformed review reference; the prose has not been judged invalid."""
...
def validate_authority(data, plan, user_input, previous):
    ...
    for key in ('checks', 'stateChecks'):
        entries = data.get(key)
        if isinstance(entries, list):
            problems.extend(str(e.get('reason', '未说明授权')) for e in entries
                            if isinstance(e, dict) and e.get('authorized') is False)
        ...
        for entry in entries:
            ...
            if key == 'checks':
                ...
                if quote not in source:
                    raise ActionEvidenceError('行动授权引文不在原输入或前文中：' + entry['stepId'])
    if problems or data['decision'] != 'allow':
        raise ValueError('行动计划越权：' + '；'.join(problems[:4]))
    return data
...
def attribute_vocabulary(package, state, plan):
    ...
    for entity, item in registry(package, state, plan['introductions']).items():
        fields = ('locationId', 'ownerCharacterId', item_lifecycle.ATTRIBUTE) if item['kind'] == 'item' else ('locationId', 'outcome') if item['kind'] == 'character' else ()
...
def require_available_actor(state, actor, outcomes=()):
    outcome = state.get('characterOutcomeStates', {}).get(actor, {})
    if outcome.get('permanence') == 'permanent' and outcome.get('status') in ('dead', 'departed'):
        raise ValueError('永久下线人物不能继续承担当前行动或发言：' + actor)
    if outcome.get('status') == 'missing' and not any(isinstance(o, dict) and o.get('characterId') == actor and o.get('status') in ('alive', 'injured') for o in outcomes or []):
        raise ValueError('失踪人物须先以已核对的重现结果恢复，不能直接参与行动：' + actor)
...
def validate_plan(plan, context):
    ...
    for step in steps:
        ...
        require_available_actor(context['parent']['branchState'], step['actorId'], plan.get('outcomes', []))
    ...
    for c in changes:
        ...
        if attr in ('id', 'kind', 'name', 'outcome', 'goal', 'thread', 'characterOutcomeStates', 'goalLedger', 'threadLedger', STATE_KEY):
        ...
        if attr in ('locationId', 'ownerCharacterId'):
            ...
            if kind == 'character' and outcome.get('status') == 'missing' and value is not None:
                require_available_actor(context['parent']['branchState'], entity, plan.get('outcomes', []))
    ...
    item_lifecycle.validate_changes(context['parent']['branchState'], plan, known)
    return plan


def project(state, plan, package):
    from .cocreation import apply_branch_patch
    item_lifecycle.validate_changes(state, plan, registry(package, state, plan['introductions']))
    ...
    item_lifecycle.clear_destroyed_positions(result)
    return result


class ObservationCoverageError(ValueError):
    """Valid extracted events with explicitly identified missing paragraphs."""
    def __init__(self, events, missing):
        self.events, self.missing = events, missing
        super().__init__('事实提取必须覆盖每个正文段落；缺少：' + ','.join(missing))


def merge_observations(existing, supplement):
    events = sorted(existing + supplement, key=lambda e: int(e['paragraphId'][1:]))
    return {'events': [{**e, 'id': f'O{i+1}'} for i, e in enumerate(events)]}


def validate_observations(data, body, required_paragraphs=None):
    paragraphs = {f'P{i+1}': p for i, p in enumerate(body.split('\n\n'))}
    expected = set(paragraphs) if required_paragraphs is None else set(required_paragraphs)
    if not expected or not expected <= set(paragraphs):
        raise ValueError('事实提取的段落范围无效')
    ...
    for original in events:
        ...
        if not p or e.get('paragraphId') not in expected:
        ...
        if not isinstance(e.get('usedItemIds', []), list) or any(not isinstance(iid, str) for iid in e.get('usedItemIds', [])):
            raise ValueError('正文使用的道具清单格式无效')
    ...
    if covered != expected:
        raise ObservationCoverageError(grounded, sorted(expected-covered, key=lambda p: int(p[1:])))
...
def validate_events(review, events, plan, state=None, package=None):
    ...
    for event in events:
        ...
        if event.get('mode') in ('speech', 'intention'):
            actor = event.get('actor')
            if actor not in known:
                matches = [cid for cid, entity in known.items()
                           if entity['kind'] == 'character' and entity['name'] == actor]
                actor = matches[0] if len(matches) == 1 else actor
            require_available_actor(state or {}, actor, plan.get('outcomes', []))
    observed_values = {}
    destruction_evidence = set()
    unavailable_items = {iid for iid, entity in known.items() if entity['kind'] == 'item' and item_lifecycle.destroyed(state or {}, iid)}
    for event in events:
        check = by_id[event['id']]
        for field, allowed in (('stepIds', steps), ('changeIds', changes), ('introductionIds', entities)):
            ...
            if not isinstance(refs, list) or any(not isinstance(ref, str) or ref not in allowed for ref in refs):
                raise ActionEvidenceError(f"事件 {event['id']} 的 {field} 引用无效：{refs!r}；仅允许 {sorted(allowed)!r}，无对应变化应返回空数组；不得补造契约条目")
        if event['introduced'] and not check['introductionIds']:
            raise ValueError('正文引入未登记的关键实体：' + '、'.join(event['introduced']))
        if event.get('mode') in ('actual', 'background'):
            uses = event.get('usedItemIds', [])
            if any(known.get(iid, {}).get('kind') != 'item' for iid in uses):
                raise ValueError('正文使用了未登记的道具')
            if unavailable_items.intersection(uses):
                raise ValueError('正文不能使用已永久损毁的道具')
        for observed in event['changes']:
            if not isinstance(observed, dict):
                raise ValueError('提取的状态变化格式无效')
            key = (observed.get('entityId'), observed.get('attribute'))
            if key[0] in unavailable_items and event.get('mode') in ('actual', 'background'):
                if (key[1] == item_lifecycle.ATTRIBUTE and observed.get('value') is not True
                        or key[1] in ('locationId', 'ownerCharacterId') and observed.get('value') is not None):
                    raise ValueError('正文不能恢复或重新持有已永久损毁的道具')
            if key[1] == item_lifecycle.ATTRIBUTE:
                if observed.get('value') is not True:
                    raise ValueError('永久损毁事实必须使用布尔 true')
                if event.get('mode') == 'actual':
                    destruction_evidence.update(r for r in check['changeIds']
                        if changes[r]['entityId'] == key[0] and changes[r]['attribute'] == key[1]
                        and changes[r]['value'] is True)
            if key[1] == item_lifecycle.ATTRIBUTE and observed.get('value') is True and event.get('mode') == 'actual':
                unavailable_items.add(key[0])
    ...
    required = {c['id'] for c in changes.values() if c['attribute'] == item_lifecycle.ATTRIBUTE
                and not item_lifecycle.destroyed(state or {}, c['entityId'])}
    if required - destruction_evidence:
        raise ValueError('永久损毁缺少独立提取并审查的实际正文事件')
    return checks
