...
def journey(store, sid, bid, package):
    ...
    def effects(before, after):
        ...
        return changes
    feedback = effects(nodes[-2]['branchState'], state) if len(nodes) > 1 else []
    recap = [{'branch_id': n['id'], 'action': n.get('playerDirection') or n.get('selectedDirection', {}).get('title') or n.get('summary', ''),
              'effects': effects(nodes[i-1]['branchState'], n['branchState'])}
    ...
    return {'role_name': name, 'goal': ROLE_GOALS.get(name, '探索这段故事，走到属于你的结局') if rainy else '探索这段故事，走到属于你的结局',
