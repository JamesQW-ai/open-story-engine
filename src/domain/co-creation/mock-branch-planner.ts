...
const templates: Record<string, Template> = {
  ...
  direction_rescue_first: {
    ...
    openThreads: ["降低隧道水位", "打开信号室", "留在站务室的证据"],
    nextDirections: [{ id: "direction_lower_water_without_proof", title: "排开积水", summary: "先打开排水通路，在水位继续上涨前靠近信号室。" }],
...
const directionSourceNodes: Record<string, string> = {
  direction_rescue_first: "node_tunnel",
  direction_open_signal_room_without_proof: "node_aftermath",
};
...
export class MockBranchPlanner implements BranchPlanner {
  async plan(request: BranchPlanRequest): Promise<PlannerExecution> {
    ...
    const result = {
      ...
      ...direction,
      sourceNodeRef: directionSourceNodes[direction.id] ?? direction.sourceNodeRef,
